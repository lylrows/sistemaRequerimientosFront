...
export class FormSistemasComponent implements OnInit {
  ...
  LoadSystem() {
    this.seguridadService.getActividadByIdMejora().subscribe(res =>{
      this.systemList=res.objModel.filter(x=>x.idMejora==this.data.id);
      //console.log("actividades ",res.objModel)
    })
...
let user = JSON.parse(localStorage.getItem('currentUser'));
    ...
    this.mejorasSolucion_.idMejora=this.data.id;
...
let user = JSON.parse(localStorage.getItem('currentUser'));
    ...
    this.mejorasSolucion_.fechaActualiza= date._d;
    this.mejorasSolucion_.esActivo= 1;    
    ////console.log("Solucion",this.mejorasSolucion_);
  ...
  confirmDelete(){
    ...
    this.mejorasSolucion_.esActivo= this.infoDelete.esActivo 
     /*this.seguridadService.deleteMejoraActividad(this.mejorasSolucion_).subscribe(res=>{
       this.LoadSystem();
       this.showNotification(
        'snackbar-info',
