...
export class EditDepartmentComponent   implements OnInit{
  ...
  LoadParameters() {
    ...
    this.departmentService.getNivelSoporteById(user.id).subscribe(res =>{
      ...
      //console.log("sistemaEmpresas", res.objModel)
  ...
  editar(){
    if(this.departmentService.mejoraDTO.mejora.idEstado != 2 && this.departmentService.mejoraDTO.mejora.idEstado != 4){
      Swal.fire({
        position: 'center',
        icon: 'warning',
        title: 'Atención',
        text:'El registro debe estar "Aprobado" ó "Rechazado" para editar.',
